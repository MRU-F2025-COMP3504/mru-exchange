...
export default function MessagingPage() {
    ...
    const { loading: chatsLoading, chats } = useChats({ supabase_id: currentUserId });
    ...
    const ChatCard = ({ chat, showRemove = false, showSold = false }: { chat: Chat; showRemove?: boolean; showSold?: boolean }) => {
            ...
            <div className="bg-[#F9FAFB] min-h-screen">
                ...
                <main className="flex items-center justify-center min-h-[60vh]">
                    <div className="text-center">
                        <p className="text-xl text-gray-600 mb-4">Please log in to view your chats</p>
                        <button
                    ...
                    chats.map(chat => (
                        <div
                            ...
                            style={{
                                ...
                                borderRadius: '8px',
                                cursor: 'pointer',
                                backgroundColor: chat.id === selectedChat ? '#e5e7eb' : 'transparent'
                            }}
                        >
                            Chat #{chat.id}
                        </div>
                            ...
                            {chatHook?.messages?.length ? (
                                chatHook.messages.map((msg) => (
                                    <div key={msg.id} style={{ marginBottom: '1rem' }}>
                                        <strong>{msg.sender_id === currentUserId ? 'You' : msg.sender_id}</strong>: {msg.logged_message}
                                    </div>
                                ))
                            ) : (
                                <p>No messages yet</p>
