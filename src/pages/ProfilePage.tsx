...
export default function ProfilePage() {
    ...
    const [activeTab, setActiveTab] = useState<'bookmarks' | 'listings'>('bookmarks');
    ...
    const handleMarkAsSold = async (productId: number) => {
        ...
        try {
            ...
            setBookmarkedProducts(prev => 
                prev.map(p => 
                    p.id === productId 
                        ? { ...p, isDeleted: true, isListed: false }
                        : p
                )
            );
    ...
    const ProductCard = ({ product, showRemove = false, showSold = false }: { product: Product; showRemove?: boolean; showSold?: boolean }) => {
        ...
        return (
            ...
            >
                ...
                <div className="p-4">
                    ...
                    {product.price !== null && (
                        <p className={`font-bold text-xl mb-2 ${isSold ? 'text-gray-400 line-through' : 'text-[#007FB5]'}`}>
                            ${product.price.toFixed(2)}
                        </p>
                    )}
                    <p className="text-gray-600 text-sm line-clamp-2">{product.description}</p>
                    {!isSold && product.stock_count !== null && (
                        <p className={`text-sm mt-2 ${product.stock_count > 0 ? 'text-green-600' : 'text-red-600'}`}>
                        ...
                        <div className="flex-1">
                            ...
                            {userInfo && (
                                <div className="space-y-2">
                                    <p className="text-lg">
                                        <span className="font-semibold">Name:</span> {userInfo.first_name} {userInfo.last_name}
                                    </p>
                                    {userInfo.user_name && (
                                        <p className="text-lg">
                                            <span className="font-semibold">Username:</span> {userInfo.user_name}
                                        </p>
                                    )}
                                    <p className="text-lg">
                ...
                {activeTab === 'bookmarks' ? (
                    <div>
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-2xl font-bold text-gray-800">Bookmarked Products</h2>
                        </div>
                        {bookmarkedProducts.length === 0 ? (
                            <div className="bg-white rounded-lg shadow p-8 text-center">
                                <p className="text-gray-600 mb-4">You haven't bookmarked any products yet</p>
                                <button
