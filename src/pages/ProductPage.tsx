...
export default function ProductPage() {
    ...
    const fetchProductReviews = async () => {
        try {
            ...
            const transformedReviews = reviewsData?.map((review: any) => ({
                ...
                reviewer_name: `${review.User_Information?.first_name || ''} ${review.User_Information?.last_name || ''}`.trim() || 'Anonymous'
            })) || [];
    ...
    const linkData: Array<LinkData> = [
        { name: 'Product Search', path: '/product-search', className: 'hover:text-[#0F76D7]' },
        { name: product.title || 'Product', path: '#', className: 'hover:text-[#0F76D7]' },
    ];
            ...
            <main>
                <section id="details" className="p-8 bg-[#A7E2FC] text-[#003A5F]">
                    ...
                    <section className="p-5 flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="lg:w-2/3">
                            ...
                            <p className={`text-lg mb-4 font-semibold ${isOutOfStock ? 'text-red-600' : 'text-green-600'}`}>
                            ...
                            {bookmarkSuccess && (
                                <div className="mt-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg">
                                    <p className="font-semibold">Product bookmarked successfully!</p>
                                </div>
                            )}
                ...
                <section className="p-10">
                    ...
                    <p className="text-2xl mb-6">
                        <span className="text-3xl">{displayStars(avgRating)}</span> ({reviews.length} {reviews.length === 1 ? 'review' : 'reviews'})
                    </p>
                            ...
                            {reviews.map((review) => (
                                <div key={review.id} className="bg-white p-4 rounded-lg shadow">
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <p className="text-yellow-500">{displayStars(review.rating)}</p>
                                    </div>
                                    {review.description && (
